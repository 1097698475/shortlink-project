...
public interface LinkBaseStatsMapper extends BaseMapper<LinkBaseStatsDO> {
     ...
     * - 这样可以得到这个短链接的每天总的访问量（PV）、独立访客数（UV）、独立 IP 数（UIP）
     *
    ...
    /**
     * 查询分组内所有短链接在日期范围内的日粒度统计数据
     * 累计分组内短链接的数据
     *
     * SQL 功能说明：
     * - SUM(pv/uv/uip)：对同一天的数据累加
     * - GROUP BY date：按日期聚合（每天）
     * - 这样可以得到这个gid分组的每天总的访问量（PV）、独立访客数（UV）、独立 IP 数（UIP）
     *
    ...
    @Select("""
            ...
            WHERE
                ...
                AND date BETWEEN #{param.startDate} AND #{param.endDate}
            GROUP BY
                date;  -- 这样写最简洁，教程是gid, hour，前1个字段冗余了
            """)
    List<LinkBaseStatsDO> listDayStatsByGroup(@Param("param") ShortLinkGroupStatsReqDTO requestParam);
    ...
    /**
     * 查询分组内所有短链接在日期范围内的小时粒度统计数据
     * 累计分组内短链接的数据，聚合成一个“分组”数据
     *
     * SQL 功能说明：
     * - SUM(pv)：对每小时的数据累加
     * - GROUP BY hour：按小时聚合
    ...
    @Select("""
            ...
            GROUP BY
                hour;
            """)
    List<LinkBaseStatsDO> listHourStatsByGroup(@Param("param") ShortLinkGroupStatsReqDTO requestParam);
    ...
    @Select("""
            ...
            GROUP BY
                weekday;
            """)
    List<LinkBaseStatsDO> listWeekdayStatsByGroup(@Param("param") ShortLinkGroupStatsReqDTO requestParam);
}
