...
import static org.lin1473.shortlink.admin.common.constant.RedisCacheConstant.LOCK_USER_REGISTER_KEY;
import static org.lin1473.shortlink.admin.common.enums.UserErrorCodeEnum.*;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, UserDO> implements UserService {
    ...
    public UserLoginRespDTO login(UserLoginReqDTO requestParam) {
        ...
        UserDO userDO = baseMapper.selectOne(queryWrapper);
        if (userDO == null) {
            throw new ClientException("用户不存在");     // 返回一个基本的ErrorCode: CLIENT_ERROR，密码错误也返回这个
        }
         ...
         *  Val：JSON 字符串（用户信息）
         */
        String uuid = UUID.randomUUID().toString();         // 生成UUID唯一标识，作为redis的Key
